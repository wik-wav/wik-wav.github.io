...
### Canonical identity and authorship evidence

The public origin is intentionally locked in content validation and is read-only in Portfolio Studio. Every production route emits exactly one absolute canonical URL, a matching Open Graph URL, `rel="author"`, `rel="me"` links to the configured official profiles, and a connected JSON-LD graph containing the same `Person`, `WebSite`, and page identity. The sitemap is generated from the same route manifest and its URL set is regression-tested against those page canonicals.

These signals make the original site unambiguous to search engines and preserve a consistent identity if the rendered frontend is copied. They do not technically prevent someone from downloading public HTML, images, or rewriting a clone. Keep the repository's existing commit history, authored content dates, and provenance fields; do not squash or force-rewrite published history. Signed commits and signed release tags for major portfolio versions provide stronger timestamped evidence than build dates, which are intentionally not invented in page metadata.

Repository privacy is safe for normal public viewing only when the owning GitHub plan supports Pages from private repositories. GitHub Pages supports private repositories on GitHub Pro, Team, and Enterprise plans; changing a Pages repository to private on GitHub Free automatically unpublishes the site. Confirm the account plan and a successful Pages deployment before changing visibility. Changing a public repository to private also does not retract existing public forks or copies. See [What is GitHub Pages?](https://docs.github.com/en/pages/getting-started-with-github-pages/what-is-github-pages) and [Setting repository visibility](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/managing-repository-settings/setting-repository-visibility).
