(() => {
  ...
  const state = { bootstrap: null, type: null, id: null, record: null, revision: null, mediaKey: "cover", mediaItemIndex: 0, mediaUploadMode: "replace", dirty: false, createType: null, archiveTarget: null, typographyTarget: null };
  ...
  function renderLibrary() {
    ...
    $('[data-list="projects"]').innerHTML = containersForList.map(item => {
      ...
      const peers = containersForList.filter(entry => entry.recordType === type);
      const peerIndex = peers.findIndex(entry => entry.id === item.id);
      const selected = state.type === type && state.id === item.id;
      return '<li><button type="button" class="library-item" data-open="' + esc(type) + '" data-id="' + esc(item.id) + '" aria-current="' + selected + '"><strong>' + esc(titleOf(item)) + "</strong><small>PROJECT · " + esc(item.id) + (item.draft ? " · DRAFT" : "") + '</small></button><span class="order-buttons"><button type="button" data-move="up" data-type="' + esc(type) + '" data-id="' + esc(item.id) + '" aria-label="Move up"' + (peerIndex === 0 ? " disabled" : "") + '>↑</button><button type="button" data-move="down" data-type="' + esc(type) + '" data-id="' + esc(item.id) + '" aria-label="Move down"' + (peerIndex === peers.length - 1 ? " disabled" : "") + ">↓</button></span></li>";
    }).join("");
    ...
    $('[data-list="works"]').innerHTML = works.map(item => {
      ...
      const mediaBadge = item.mediaType === "video" ? (["soundcloud", "bandcamp"].includes(item.video?.provider) ? "AUDIO" : "VIDEO") : "";
  ...
  function mediaFor(record, key) {
    return record[key] || fallbackMedia();
  }
  ...
  function renderTagEditor(record) {
    ...
    const known = Array.from(new Set([...(state.bootstrap.taxonomy?.workTypes || []), ...selected])).sort((a, b) => a.localeCompare(b, "en"));
  ...
  function renderContainer(record) {
    ...
    return '<div class="editor-head"><div><p class="overline">PROJECT</p><h2>' + esc(titleOf(record)) + "</h2>" + (record.draft ? '<span class="draft-mark">DRAFT · NOT PUBLIC</span>' : "") + '</div><button type="button" class="button secondary" data-duplicate>Duplicate</button></div>' +
      ...
      '<fieldset><legend>Introduction</legend><div class="field-grid">' + pair("summary","Short summary",record,{area:true}) + pair("overview","Overview",record,{area:true}) + pair("year","Year",record) + pair("disciplines","Disciplines",record) + pair("format","Format",record) + pair("role","Role / scope",record,{area:true}) + "</div></fieldset>" +
      renderMediaEditor(record,["cover","hero","thumbnail"]) +
      '<fieldset><legend>Process & credits</legend><div class="field-grid">' + pair("processHeading","Section heading",record) + pair("process","Process",record,{area:true}) + pair("credits","Credits",record,{area:true}) + pair("statement","Optional statement",record,{area:true}) + "</div></fieldset>" +
  ...
  function renderEditor() {
    ...
    form.hidden = false;
    const content = state.type === "site" ? renderSite(state.record) : state.type === "work" ? renderWork(state.record) : renderContainer(state.record);
  ...
  async function uploadImage(file) {
    try {
      ...
      if (mediaRoot.kind === "group") {
        ...
        state.record[state.mediaKey] = mediaRoot;
      } else state.record[state.mediaKey] = payload.media;
  ...
  async function reorder(type,id,direction) {
    const list = state.bootstrap.summaries[type + "s"];
    const index = list.findIndex(item => item.id === id);
    const next = direction === "up" ? index - 1 : index + 1;
    if (next < 0 || next >= list.length) return;
    const ids = list.map(item => item.id);
    [ids[index],ids[next]] = [ids[next],ids[index]];
    try { await api("/api/order/" + type,{ method:"PUT",body:{ids} }); await refreshBootstrap(); }
    catch (error) { showError(error); }
  }
  ...
  document.addEventListener("click",event => {
    ...
    if (move) reorder(move.dataset.type,move.dataset.id,move.dataset.move);
